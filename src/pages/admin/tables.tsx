...
export default function Orders() {
  ...
  const detail = (record: any) => {
    axios.get("/api/table/" + record.id).then(({ data }) => {
      setEdit(data.data);
    }).catch(() => {
  ...
  const deleteRecord = (record: any) => {
    axios.delete("/api/table/" + record.id).then(({ data }) => {
      getData();
  ...
  const save = (data: any) => {
    if (!edit.id) {
      axios.post("/api/table/create", data).then(({ data }) => {
        getData();
        setEdit(null);
      ...
      axios.put("/api/table/" + edit.id, data).then(({ data }) => {
        getData();
        setEdit(null);
        if (edit.id) toast.success("Kayıt başarıyla güncellendi");
        else toast.success("Kayıt başarıyla eklendi");
