...
const EditModal = ({ edit, setEdit, save, categories }: any) => {
  const schema = yup.object().shape({
    title: yup.string().required("Başlık zorunlu"),
    price: yup.string().required("Fiyat zorunlu"),
    image: yup.string().required("Resim zorunlu"),
                    ...
                    <div className="mt-3 w-full text-center sm:ml-4 sm:mt-0 sm:text-left">
                      ...
                      <div className="relative mt-4">
                        ...
                        <select
                          {...register("categoryId")}
                          id="categoryId"
                          className={`cursor-pointer w-full rounded border-gray-300 bg-gray-50 py-3 px-2 text-sm shadow-sm outline-none transition focus:ring-2 focus:ring-teal-500
                      ${errors["categoryId"] && "!ring-red-600"}
                      `}
                        >
                            <option value="">
                              Kategori seçin
...
export default function Orders() {
  ...
  useEffect(() => {
    getData();
    axios.get("/api/category").then(({ data }) => setCategories(data.data));
  }, []);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <img src={text} className="w-10 h-10 object-cover" alt="" />
      ),
    },
    {
      title: "Başlık",
  ...
  return (
    <AdminLayout>
      {!!edit && <EditModal categories={categories} save={save} edit={edit} setEdit={setEdit} />}
